...
if phase == 0
    ...
    if ~exist(fileName, 'file') % Check to see if the subject has already done the preliminary phase or not
        ...
        if ~exist(directory, 'dir') % Check the directory actually exists
            mkdir(directory);
            fileName = sprintf('%s%s-GaborTestContrast.mat',directory,subjectID); % create a name for the data you want to save
...
elseif phase == 1
    ...
    if ~exist(fileName, 'file') % Check to see if the subject has already done the preliminary phase or not
        ...
        if ~exist(directory, 'dir') % Check the directory actually exists
            mkdir(directory);
            fileName = sprintf('%s%s-GaborTestRatio.mat',directory,subjectID); % create a name for the data you want to save
            save(fileName, 'Test_Data','test_image_collection'); % save the data
        else
            fileName = sprintf('%s%s-GaborTestRatio.mat',directory,subjectID); % create a name for the data you want to save
            save(fileName, 'Test_Data','test_image_collection'); % save the data
