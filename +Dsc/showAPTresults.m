...
cond = 3;
resFolder = ['../dscData/resultsSyntheticDataCB_debug'];
resNames = dir([resFolder,'/res*.mat']);
%%
...
subplot(2,4,[1:4])
plot(log{1}.loss);
subplot(2,4,5);histogram(posterSamples(:,1));hold on;line(groundTruth.priorC*[1,1],[0,50],'color','red');hold off
subplot(2,4,6);histogram(posterSamples(:,2));hold on;line(groundTruth.gamma*[1,1],[0,50],'color','red');hold off
subplot(2,4,7);histogram(posterSamples(:,3));hold on;line(groundTruth.lapse*[1,1],[0,50],'color','red');hold off
subplot(2,4,8);histogram(posterSamples(:,4));hold on;line(groundTruth.samples*[1,1],[0,50],'color','red');hold off
pause()
