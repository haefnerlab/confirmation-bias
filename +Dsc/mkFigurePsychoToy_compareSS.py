...
# -*- coding: utf-8 -*-
"""
Created on Tue May  5 21:51:51 2020
...
para_discrepancy_choice = np.zeros((2,len(samples_list),9,5))
para_discrepancy_signal = np.zeros((2,len(samples_list),9,5))

for i,N_trial in enumerate(N_trial_list):
    resultsFolder = '../dscData/Psycho Results/Average Statistics/selfConsistencyOverParamDraws%d' % N_trial
        ...
        for file in os.listdir(subfolder):
            ...
            if file.endswith('.pk') & file.startswith('posterior_apt'):
                with open(os.path.join(subfolder,file),'rb') as handle:
                    ...
                    para_discrepancy_choice[i,j,n,:] = calculate_discrepancy(apt_samples,analytical_samples,option = 'parametric')
                    n += 1          
        ...
        for file in os.listdir(subfolder):
            ...
            if file.endswith('.pk') & file.startswith('posterior_apt'):
                with open(os.path.join(subfolder,file),'rb') as handle:
                    ...
                    #nonpara_discrepancy_apt[i,j,:,n1] = calculate_discrepancy(apt_samples,analytical_samples,option = 'nonparametric')
                    para_discrepancy_signal[i,j,n,:] = calculate_discrepancy(apt_samples,analytical_samples,option = 'parametric')
                    n += 1   
#%%  
  
logpara_discrepancy_choice = np.log(para_discrepancy_choice)
logpara_discrepancy_signal = np.log(para_discrepancy_signal)
...
for i in range(5):
    ...
    axs[i].errorbar(np.log(np.asarray(samples_list)*2),y = np.mean(logpara_discrepancy_choice[0,:,:,i],axis = 1),\
                  yerr = np.std(logpara_discrepancy_choice[0,:,:,i],axis = 1),label ='choice_20',color = 'r',ls= '--')
        
    axs[i].errorbar(np.log(np.asarray(samples_list)*2),y = np.mean(logpara_discrepancy_signal[0,:,:,i],axis = 1),\
                  yerr = np.std(logpara_discrepancy_signal[0,:,:,i],axis = 1),label = 'signal_20',color = 'y',ls = '--')
    
    axs[i].errorbar(np.log(np.asarray(samples_list)*2),y = np.mean(logpara_discrepancy_choice[1,:,:,i],axis = 1),\
                  yerr = np.std(logpara_discrepancy_choice[1,:,:,i],axis = 1),label ='choice_50',color = 'r')
        
    axs[i].errorbar(np.log(np.asarray(samples_list)*2),y = np.mean(logpara_discrepancy_signal[1,:,:,i],axis = 1),\
                  yerr = np.std(logpara_discrepancy_signal[1,:,:,i],axis = 1),label = 'signal_50',color='y')    
