...
distributions = struct(...
    ...
    'gamma', uniform(0, 1, .1), ...
    'sensor_noise', gammavar(1, 3, 100), ...
    'step_size', gammavar(2, .05/2, 100), ...
    'var_x', gammavar(1, 1/10, 100), ...
    'noise', gammavar(1, 1/4, 100), ...
    'temperature', gammavar(1, 8, 100), ...gammavar(3/2, 1/3, 100), ...
    ...
    for iF=1:length(fields)
        ...
        if islog(iF)
            ...
            if is_map
                ...
                distrib.proppdf = @(logx1,logx2) olddistrib.proppdf(exp(logx1), exp(logx2));
                distrib.logpriorpdf = @(logx) olddistrib.logpriorpdf(exp(logx));
                if isfield(distrib, 'logproppdf')
                    distrib.logproppdf = @(logx1,logx2) olddistrib.logproppdf(exp(logx1), exp(logx2));
                end
            else
                distrib.priorpdf = @(logx) olddistrib.priorpdf(exp(logx)).*exp(logx);
                distrib.proppdf = @(logx1,logx2) olddistrib.proppdf(exp(logx1),exp(logx2)).*exp(logx1);
                distrib.logpriorpdf = @(logx) olddistrib.logpriorpdf(exp(logx)) + logx;
                if isfield(distrib, 'logproppdf')
                    distrib.logproppdf = @(logx1,logx2) olddistrib.logproppdf(exp(logx1), exp(logx2)) + logx1;
                end
            end
        end
...
distrib = struct(...
    ...
    'proppdf', @(x1, x2) double(x1>=max(a,x2-range/2) & x1<min(b,x2+range/2))./(min(b,x2+range/2) - max(a,x2-range/2)), ...
